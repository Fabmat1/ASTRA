...
{
public:
    /// Symlinks the per-user autoeps cache into `workDir`, so ISIS reuses the
    /// rendered plots it already has instead of rebuilding them for every fit.
    /// Returns the cache path, or an empty string if the link could not be
    /// made (in which case ISIS simply rebuilds, so this is not fatal).
    /// `onLog`, when set, receives a short status or warning line.
    static QString ensureAutoepsCache(
