...
double chi2Critical(int dof, double thresh) {
    ...
    while (Stats::logChi2SF(hi, dof) > thresh) {
        hi *= 2.0;
        if (hi > 1e12) return std::numeric_limits<double>::infinity();
    }
    for (int i = 0; i < 200; ++i) {
        const double mid = 0.5 * (lo + hi);
        if (Stats::logChi2SF(mid, dof) > thresh) lo = mid; else hi = mid;
    }
    return 0.5 * (lo + hi);
}
    ...
    for (long long it = 0; it < trials; ++it) {
        ...
        if (!mdl.eccentric) {
            ...
            for (std::size_t i = 0; i < n; ++i) {
                ...
                u -= std::floor(u);
                const double nu = RVFit::trueAnomaly(
                    RVFit::solveKepler(TWO_PI * u, e), e);
