...
double RadialVelocityCurve::computeLogP() const
{
    ...
    // See Stats::logChi2SF() for the continued-fraction implementation.
    double logp = Stats::logChi2SF(chisq_sum, dof);
...
double RVFit::solveKepler(double M, double e, double tol, int maxIter)
{
    ...
    if (!(e > 0.0)) return M;               // circular orbit: E == M

    // Newton alone is not safe here. Kepler's equation has f'(E) = 1 - e cos E,
    // which approaches 1 - e near E = 0, so for e close to 1 a single Newton
    // step from the wrong side throws the iterate thousands of radians away and
    // it never returns: the old version started every e >= 0.8 orbit at E = π
    // and diverged for M below about -1.8, returning E = -8668 for
    // (M = -3, e = 0.99). That is roughly 7 % of the orbit at e = 0.99.
    //
    // E - M = e sin E, so the root always lies in [M - e, M + e], and f is
    // strictly increasing there for e < 1, so it is a genuine bracket. Keeping
    // it and falling back to bisection whenever Newton would step outside makes
    // convergence unconditional while retaining Newton's quadratic rate in the
    // ordinary case.
    double lo = M - e;
    double hi = M + e;
    double E  = (e < 0.8) ? (M + e * std::sin(M)) : M;
    ...
    for (int i = 0; i < maxIter; ++i) {
        ...
        double       next = (fp != 0.0) ? (E - f / fp)
                                        : std::numeric_limits<double>::quiet_NaN();
        if (!(next > lo && next < hi))
            next = 0.5 * (lo + hi);         // Newton left the bracket

        const double dE = next - E;
        E = next;
...
double RVFit::trueAnomaly(double E, double e)
{
