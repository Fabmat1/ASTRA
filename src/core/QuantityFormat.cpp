...
QString formatEta(qint64 milliseconds)
{
    ...
    if (seconds < 90)
        return QObject::tr("~%1 s remaining").arg(std::max<qint64>(seconds, 1));
    if (seconds < 5400)
        return QObject::tr("~%1 min remaining").arg((seconds + 30) / 60);
    return QObject::tr("~%1 h remaining").arg((seconds + 1800) / 3600);
}
