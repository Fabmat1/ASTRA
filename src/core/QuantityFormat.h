...
/// A coarse "time remaining" label for a long-running job: seconds below a
/// minute and a half, then minutes, then hours. Deliberately imprecise - the
/// underlying estimate is not good enough to justify a finer reading, and a
/// figure that visibly jitters reads as less trustworthy than a rounded one.
/// Returns an empty string for a negative (that is, unknown) duration.
QString formatEta(qint64 milliseconds);
