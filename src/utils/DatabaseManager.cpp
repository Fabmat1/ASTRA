...
QString DatabaseManager::instrumentUUID(const QString& name)
{
...
void DatabaseManager::loadDefaultInstruments()
{
...
void DatabaseManager::loadInstrumentsFromDatabase()
{
...
bool DatabaseManager::writeInstrumentToDb(const Instrument& inst)
{
    QSqlQuery q(threadConnection());
    q.prepare(R"(
...
bool DatabaseManager::writeModesToDb(const QString& instrumentId,
                                     const QList<InstrumentMode>& modes)
{
    QSqlQuery dq(threadConnection());
...
void DatabaseManager::cacheInstrument(std::shared_ptr<Instrument> inst)
{
    _instrumentsById[inst->getId()] = inst;
    _instrumentsByName[inst->getName().toLower()] = inst;
}

void DatabaseManager::uncacheInstrument(const QString& id)
{
...
// ── Compound key resolution (for point d) ───────────────────────────────────

std::shared_ptr<Instrument> DatabaseManager::resolveInstrumentString(
