...
// The epoch arithmetic (and the reasoning behind it) lives in ExposureEpoch.h
// so it can be tested without a FITS fixture; these wrappers only pull the
// cards out of the header.
double mjdFromKey(fitsfile* fptr, const char* key) {
    ...
    return SpecFetch::lamostMjdFromDateString(
        QString::fromLatin1(val).trimmed());
}
...
double exposureMidMjd(fitsfile* fptr, double localMinuteStamp,
                      double exposureSec) {
    return SpecFetch::lamostExposureMidMjd(
        mjdFromKey(fptr, "DATE-OBS"),      // already UTC
        mjdFromKey(fptr, "DATE-BEG"),      // Beijing
        mjdFromKey(fptr, "DATE-END"),      // Beijing
        localMinuteStamp,
        exposureSec);
}
...
static std::vector<SpecFetch::ParsedSpectrum> parseSexp(
    ...
    for (const QString& epoch : epochOrder) {
        ...
        if (mjmVal > 0)
            spec->setMJD(SpecFetch::lamostExposureMidMjd(
                NAN, NAN, NAN, mjmVal, expSec));
