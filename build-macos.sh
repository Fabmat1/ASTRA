...
#
# CCfits is REQUIRED (both ASTRA and GAEL link it); it comes from the
...
# gcc -> gfortran (GAEL declares `LANGUAGES ... Fortran`, so CMake needs a
#                  Fortran compiler at configure time even with no .f sources)
# libomp -> OpenMP for Apple Clang (GAEL and the RV-MCMC sampler require it)
...
# cxxopts -> GAEL (its brew formula ships the CMake config GAEL looks for)
...
# ── 4. OpenBLAS (GAEL needs the OpenBLAS::OpenBLAS CMake target) ────────────
...
if [[ -n "${OPENBLAS_BREW}" ]] && find "${OPENBLAS_BREW}" -name 'OpenBLASConfig.cmake' 2>/dev/null | grep -q .; then
  ...
  if [[ -f "${STAMP}" ]]; then
    ...
    # GAEL and ASTRA still get their own OpenMP (libomp) independently.
...
# GAEL does `find_package(OpenBLAS REQUIRED)` then links OpenBLAS::OpenBLAS,
...
# Auto-generated by build-macos.sh — defines OpenBLAS::OpenBLAS for GAEL.
...
# ASTRA resolves FFTW3 through pkg-config, but anything in the tree that asks
# for it via find_package gets a real `fftw3` imported target here (Module mode
# wins over brew's config) so FFTW's header and full library path propagate —
# no global -I/-L needed, which keeps Homebrew's lib dir off the link line (it
# can shadow the system libc++ and cause undefined __hash_memory).
FFTW_PREFIX="$(brew --prefix fftw)"
cat > "${CMAKE_MODULES}/FindFFTW3.cmake" <<EOF
# Auto-generated by build-macos.sh — provides a bare \`fftw3\` target (plus
# FFTW3::fftw3), with include/lib resolved under Homebrew.
if(NOT TARGET fftw3)
...
# ── 5. CCfits (REQUIRED — ASTRA and GAEL both link it) ─────────────────────
# GAEL does `find_library(CCfits ... REQUIRED)`, so a missing CCfits doesn't
...
# ── 6. Header-only dep: ankerl/unordered_dense (GAEL) ──────────────────────
...
# (d) GAEL: std::sqrt/std::log are constexpr only as a libstdc++ (GCC) extension;
#     Apple Clang's libc++ rejects the constexpr initializer. Use const instead.
GAEL_RES="${SRC_DIR}/external/GAEL/src/Resolution.cpp"
if grep -q 'constexpr double SIGMA_FROM_FWHM' "${GAEL_RES}"; then
  echo ">>> Patching GAEL Resolution.cpp: constexpr -> const (SIGMA_FROM_FWHM)"
  perl -i -pe 's/constexpr(\s+double\s+SIGMA_FROM_FWHM\b)/const$1/' "${GAEL_RES}"
fi
...
# The deps cache dir must be a CMake prefix too: GAEL locates unordered_dense.h
...
# macOS — the brew CCfits ships no .pc) and GAEL's `find_library(CCFITS_LIB
