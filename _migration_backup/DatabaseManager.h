...
public:
    ...
    /// Resolve a compound key like "SOAR_930" or a plain instrument name.
    /// Returns the instrument and (if present) sets modeKey.
    std::shared_ptr<Instrument> resolveInstrumentString(
        const QString& input, QString* modeKey = nullptr) const;
...
private:
    ...
    bool writeModesToDb(const QString& instrumentId,
    ...
    static QString instrumentUUID(const QString& name);
