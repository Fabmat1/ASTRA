...
#   5. fetch the git submodules (GAEL, lightcurvequery, SEDplusplus)
...
${C_BOLD}Options${C_RESET}
  ...
  -t, --build-type T    Debug|Release|RelWithDebInfo|MinSizeRel (default: ${BUILD_TYPE})
      ...
      --pinned-submodules
                        ...
                        to update GAEL, lightcurvequery, SEDplusplus
...
# 4c. FFTW3Config.cmake — ASTRA finds FFTW3 through pkg-config and falls back
#     to find_library, so this is belt-and-braces for anything else in the tree
#     that asks for the CMake package Debian/Ubuntu's libfftw3-dev omits.
ensure_fftw3_config() {
    local d
    ...
    lib="$(find_first /usr/lib/x86_64-linux-gnu/libfftw3.so /usr/lib/libfftw3.so \
        ...
        || { warn "FFTW3: libfftw3.so not found — the periodogram will fail to link."; return 0; }
...
# Rewrite any SSH submodule URL to HTTPS on the fly so the checkout works
# without a GitHub SSH key. The .gitmodules file is untouched.
git_sm() {
    git -C "${REPO_DIR}" \
...
if [[ -d "${REPO_DIR}/.git" ]]; then
    ...
    if [[ ${LATEST_SUBMODULES} -eq 1 ]]; then
        ...
        # GAEL / lightcurvequery / SEDplusplus.
    ...
    run_step "Updating submodules (GAEL, lightcurvequery, SEDplusplus)" \
        git_sm "${SUBMODULE_ARGS[@]}" \
        || die "Submodule checkout failed. Check your network connection and the log."

    for sm in GAEL lightcurvequery SEDplusplus; do
